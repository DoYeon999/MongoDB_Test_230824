...
// 인덱스 있다면 쿼리 작업 검색이 효율적

// 2. 새로운 문서 생성, 수정, 삭제 빈번시 속도 저하가 생김
// 왜냐면 매번 업데이트를 해야하기 때문

// 3. 단순 인덱스(해당 필드만 조회시 사용),
// 복합 인덱스(다수의 필드를 대상으로 조회시 사용)

// 4. 복합 인덱스는 순서가 중요
// ex) 제목 : 1, 설명 : -1 -> 이 형식으로 인덱스를 만들기때문에
// 다른 구조로 사용시 또 자원을 소모해서 사용해야함
...
// 복합키
// 다중키 : 값이 배열 형태인 곳에 설정
// 예) 문서의 속성중에서 키 : 값(배열일 떄)
// 텍스트 인덱스 : 범위에 대한 인덱스 사용하는데 문자열 검색은 범위로
// 검색을 안하고 정확한 문자열 일치여부 각 단어위주로 검색할 때 사용
// $text 연산자를 사용시 텍스트 인덱스가 필요함
// 지리 공간적 인덱스 : pdf 참고, 교재없음
...
db.rating.createIndex(
  // 시스템에서 rating 기준으로 인덱스를 만들어 둠
  // 우리는 단일키라서 해당 rating으로 검색시 효율적인 검색(빠름)
  // 1: 오름차순, -1: 내림차순
  { rating: 1 }
);
...
// 복합키 인덱스
db.rating.createIndex(
  // 의미, 1차적으로 rating 필드를 기준으로 정렬
  // 그리고나서 두번째 키 user_id를 정렬
  // 임시 예제
  // 기준이 되는 키 : rating(1)중복이 많이 되는 경우, (2)더 포괄적
  // 제약조건을 정할 때 큰 범주를 잡고 작은 범주를 선택
  { rating: 1, user_id: -1 }
);
